...
func NewSellerRepository(db *gorm.DB) *SellerGormRepository {
	return &SellerGormRepository{db: db}
}

func (r *SellerGormRepository) InsertSellers(sellers []Seller) error {
	log.Info(context.Background(), fmt.Sprintf("Attempting to insert %d new sellers...", len(sellers)))
	return r.db.Create(&sellers).Error
}

func (r *SellerGormRepository) UpdateSellers(sellers []Seller) error {
...
func (r *SellerGormRepository) GetAllSellers() ([]Seller, error) {
	var sellers []Seller
